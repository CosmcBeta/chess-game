...
Button::Button(): mouseInText_(false), text_(myriadRegular_)
{
...
Button::Button(const sf::String& string, FontType font, unsigned int characterSize,
...
: mouseInText_(false), text_(myriadRegular_), darkColor_(0, 0, 0, 255), lightColor_(0, 0, 0, 255)
{
	...
	switch (font)
	{
	case FontType::Regular : text_.setFont(myriadRegular_); break;
	case FontType::Bold	   : text_.setFont(myriadBold_); break;
	case FontType::Semibold: text_.setFont(myriadSemibold_); break;
	default				   : break;
	}
	...
	text_.setOrigin(
		{rect_.position.x + rect_.size.x / 2.f,
		 rect_.position.y + rect_.size.y / 2.f}); // Sets origin to the exact center of the text
...
void Button::update(sf::Vector2i mousePosition)
{
	...
	if (mouseInText_) text_.setFillColor(lightColor_);
	else text_.setFillColor(darkColor_);
}

bool Button::getMouseInText()
{
	return mouseInText_;
}
